...
for(var i = 0; i <=149; i++){
 ...
 function pder(){
        ...
        //if(b == 1){
            powerup = createSprite(width + 100, 180)
            powerup.scale = 0.15
            powerup.velocityX = soloi.velocityX
            gp.add(powerup)
            powerup.addAnimation("a",PT)
        //}
 }   
...
function draw() {
    ...
    if (player.isTouching(inimigogp) && boost == false){
        console.log("tocou")
        estado = "End"
    }else if(boost == true && player.isTouching(inimigogp)){
